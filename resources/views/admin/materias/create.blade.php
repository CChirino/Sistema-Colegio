...
<div class="content-error">
    <div class="hpanel">
        <div class="panel-body">
            <form method="POST" action="{{ route('materia.store') }}" id="loginForm"  >
                @csrf
                <div class="row">
                    <div class="form-group col-lg-12">
                        <label>Nombre de Materia</label>
                        <input type="text" id="dni"  class="form-control @error('nombre_materia ') is-invalid @enderror" name="nombre_materia" value="{{ old('nombre_materia ') }}" required autocomplete="nombre_materia " autofocus>
                    ...
                    <div class="form-check">
                        ...
                        type="checkbox" 
                        value="{{$pensum->id}}" 
                        id="pensum_{{$pensum->id}}" 
                        name="pensum[]">
                        <label class="form-check-label" for="pensum_{{$pensum->id}}">
                            {{$pensum->nombre}}
                        </label>
                    </div>
                    @endforeach
